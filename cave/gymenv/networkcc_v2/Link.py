...
class Link():
    ...
    def on_pkt_enter(self,ts):
        # Random loss
        if random.random() < self.trace.get_loss(ts):
            return False
        # Full queue
        if self.pkt_in_queue + 1 > self.queue_size:
            return False
    ...
    def get_latency_propagation(self, ts):
        return self.trace.get_delay(ts) / 1000.0
    
    def get_latency_queue(self,ts):
        self.pkt_in_queue = max(
            0, self.pkt_in_queue - self.trace.get_avail_bits2send(
                self.queue_delay_update_time, ts) / BITS_PER_BYTE / BYTES_PER_PACKET)
        self.queue_delay_update_time = ts

        curr_queue_delay = self.trace.get_sending_t_usage(
            self.pkt_in_queue * BYTES_PER_PACKET * BITS_PER_BYTE, ts)
        return curr_queue_delay
