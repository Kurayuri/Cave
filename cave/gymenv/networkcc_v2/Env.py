...
class Envs(gym.Env):
    ...
    def __init__(
        ...
        if self.train_flag and self.traces:
            ...
            self.real_trace_configs = np.array(
                self.real_trace_configs).reshape(-1, 4)
        else:
        ...
        if self.action_type == Sender.ACTION_CWND_DELTA:
            self.action_space = gym.spaces.Box(np.array([-1e12, -1e12]),
                                               np.array([1e12, 1e12]),
                                               dtype=np.float32)
        if self.action_type == Sender.ACTION_RATE_DELTA:
            self.action_space = gym.spaces.Box(np.array([-1e12]),
                                               np.array([1e12]),
                                               dtype=np.float32)
    ...
    def reset(self, seed=None, options=None):
        ...
        if self.train_flag and self.trace_config:
            self.current_trace = Trace.generate_trace_from_config(
                self.trace_config)
            if random.uniform(0, 1) < self.real_trace_prob and self.traces:
                config_syn = np.array(
                    self.current_trace.real_trace_configs(
                        normalized=True)).reshape(1, -1)
        ...
        self.senders = [
            Sender(10 / (self.current_trace.get_delay(0) * 2 / 1000),
        ...
        print(self.episodes_run, self.reward_ewma, self.reward_sum,
              self.steps_taken)
        self.reward_sum = 0.0
        self.steps_taken = 0
    ...
    def step(self, actions):
        ...
        reward = self.net.run_for_duration(self.timestamp_granularity,
                                           action=actions[0])
        self.steps_taken += 1
        sender_obs = self.get_sender_obs()
        terminated = self.current_trace.is_terminated(self.net.ts)
...
class Env(gym.Env):
    ...
    def __init__(
        ...
        assert traces or trace_config, "At least one of <traces> or <trace_config> is required."
    ...
    def reset(self, seed=None, options=None):
        ...
        # Newtwork
        self.net = Network(self.current_trace, sender_kwargs=dict(features=self.features, history_len=self.history_len))
        self.links = self.net.links
        self.senders = self.net.senders

        if not self.senders[0].rtt_samples:
            self.ts_granularity = 0.01

        self.episodes_run += 1
        self.net.run(self.ts_granularity)
        self.reward_ewma = 0.99 * self.reward_ewma + 0.01 * self.reward_sum
    ...
    def step(self, actions):
        for i in range(len(actions)):
            self.senders[i].act(self.action_type, actions[i])
        reward = self.net.run(self.ts_granularity)
        self.steps_taken += 1
        sender_obs = self.get_sender_obs()
        terminated = self.current_trace.is_terminated(self.net.ts)
        truncated = self.observation_space.contains(sender_obs)
